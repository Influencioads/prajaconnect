...
import { Screen, ScreenHeader, Card, PrimaryButton, Field, Badge, Chip } from '../../components/ui';
  ...
  return (
    <Screen>
      <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        {dupCheck?.hasDuplicate && (
          <Card className="mb-4 border-amber-300 bg-amber-50">
            <View className="mb-2 flex-row items-center gap-2">
              <Badge
                label={dupCheck.readOnly ? 'Duplicate blocked' : 'Duplicate warning'}
        ...
        <Card className="mb-4">
          <Field label="Name *" value={form.name} onChangeText={(v) => set('name', v)} icon="person" />
          ...
          <Field label="Age" value={form.age} onChangeText={(v) => set('age', v)} keyboardType="numeric" icon="calendar" />
          {!ageValid ? <Text className="-mt-1 mb-2 text-xs text-red-600">Age must be 0–130.</Text> : null}

          <Field label="Voter ID (EPIC)" value={form.voterId} onChangeText={(v) => set('voterId', v)} autoCapitalize="characters" icon="id-card" />
          <Field label="Occupation" value={form.occupation} onChangeText={(v) => set('occupation', v)} icon="business" />
          <Field label="Category" value={form.category} onChangeText={(v) => set('category', v)} placeholder="GEN, OBC, SC, ST…" icon="pricetag" />
